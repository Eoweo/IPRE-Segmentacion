...
class LungSegmentationDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        train_generator = load_jpg_dataset_generator(p.PATH_CT_MARCOPOLO, patient_splits, dataset_type="train")
        test_generator = load_jpg_dataset_generator(p.PATH_CT_MARCOPOLO, patient_splits, dataset_type="test")
    ...
    def val_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=10)
...
if __name__ == "__main__":
    ...
    print(f"---------------------------------------------{2 if torch.cuda.device_count() >= 2 else 1}-----------------------------------------------------------")
